...
namespace TSS.Data
{
    public class UserManagementApi : IUserManagementApi
    {
        ...
        private static readonly object _locker_Permissions = new object();

        public static UserPermissions UserPermissions
        {
            get
            {
                UserPermissions safePermissions = null;
                Interlocked.Exchange(ref safePermissions, _userPermissions);
                if (safePermissions == null || safePermissions.LoginFailed)
                {
                    _userPermissions = null;
                    lock (_locker_Permissions)
                    {
                        ...
                        safePermissions = GetUserPermissions();
                        _userPermissions = safePermissions;
                    }
                }
        ...
        {
            string requestUrl = Convert.ToString(ConfigurationManager.AppSettings["ART_API_URL"])
            ...
            catch (WebException e)
            {
                LoggerRepository.SaveLog(new Log
                {
                    ...
                    Details = "Failed to Call the ART_API Service: " + e.Message + "," + requestUrl
                },true);
                //TRY AGAIN
                if (!HasRun)
                {
        ...
        private static oAuthToken GetAccessToken()
        {
            try
            {
                var requestUrl = Convert.ToString(ConfigurationManager.AppSettings["ART_OAUTH_URL"]);
            ...
            catch (Exception e)
            {
                LoggerRepository.SaveLog(new Log
                {
                    Category = LogCategory.Application,
                    Level = LogLevel.Warning,
                    Message = string.Format("ART_OAUTH"),
                    Details = "oAuth Login  Failed:"+ e.Message+","+e.StackTrace
                },true);
        ...
        private static UserPermissions GetUserPermissions()
        {
            ...
            catch (Exception e)
            {
                LoggerRepository.SaveLog(new Log
                {
                    Category = LogCategory.Application,
                    Level = LogLevel.Warning,
                    Message = string.Format("LOAD_PERMISSIONS_FROM_LOCAL"),
                    Details = "An Exception Occured when getting the Roles in GetUserPermissions():" + e.Message
                },true); 
