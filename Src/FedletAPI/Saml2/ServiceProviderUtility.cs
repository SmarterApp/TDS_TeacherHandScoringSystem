...
namespace Sun.Identity.Saml2
{
    ...
    public class ServiceProviderUtility
    {
        ...
        public ServiceProviderUtility(HttpContext context, string configFolder)
        {
            this.Initialize(configFolder);
        }
        ...
        public ArtifactResponse GetArtifactResponse(Artifact artifact)
        {
            ...
            try
            {
                ...
                if (artifactResolutionSvcUri.Scheme == "https" && !string.IsNullOrEmpty(authCertAlias))
                {
        ...
        public void SendSoapLogoutRequest(LogoutRequest logoutRequest, string idpEntityId)
        {
            ...
            try
            {
                ...
                string authCertAlias = ConfigurationManager.AppSettings[Saml2Constants.MutualAuthCertAlias];
                if (soapLogoutSvcUri.Scheme == "https" && !string.IsNullOrEmpty(authCertAlias))
                {
                    X509Certificate2 cert = FedletCertificateFactory.GetCertificateByFriendlyName(authCertAlias);
                    if (cert != null)
                    {
        ...
        /// </param>
        public void SendLogoutResponse(HttpContext context, LogoutRequest logoutRequest, string relayState)
        {
            if (logoutRequest == null)
            {
                throw new ServiceProviderUtilityException(Resources.ServiceProviderUtilityLogoutRequestIsNull);
            ...
            if (context.Request.HttpMethod == "GET")
            {
                ...
                parameters[Saml2Constants.RelayState] = relayState;
                string redirectUrl = this.GetLogoutResponseRedirectLocation(logoutResponse, idp.EntityId, parameters);
                context.Response.Write("redirectUrl " + redirectUrl);
                context.Response.Write("relay");
                context.Response.Write("relay" + parameters[Saml2Constants.RelayState]);
                //return;
                context.Response.Redirect(relayState.ToString(), true);
            }
            else
            {
                NameValueCollection parameters = new NameValueCollection();
        ...
        public AttributeQueryResponse SendSoapAttributeQueryRequest(AttributeQueryRequest attrQueryRequest, string idpEntityId)
        {
            ...
            try
            {
                ...
                string authCertAlias = ConfigurationManager.AppSettings[Saml2Constants.MutualAuthCertAlias];
                if (soapAttributeQuerySvcUri.Scheme == "https" && !string.IsNullOrEmpty(authCertAlias))
                {
                    X509Certificate2 cert = FedletCertificateFactory.GetCertificateByFriendlyName(authCertAlias);
                    if (cert != null)
                    {
                ...
                {
                    if (string.IsNullOrEmpty(idp.EncodedEncryptionCertificate))
                    {
                        throw new ServiceProviderUtilityException(Resources.EncryptedXmlCertNotFound);
                    }

                    if (string.IsNullOrEmpty(idp.EncryptionMethodAlgorithm))
                    {
                ...
                if (string.IsNullOrEmpty(this.ServiceProvider.AttributeQuerySigningCertificateAlias))
                {
                    throw new ServiceProviderUtilityException(Resources.ServiceProviderUtilitySignFailedNoCertAlias);
                }
                else
                ...
                if (attributeQueryResponse != null && !string.IsNullOrEmpty(attributeQueryResponse.StatusCode)
...
//            throw new Saml2Exception(Resources.AuthnResponseInvalidInResponseTo);
        ...
        /// </summary>
        /// <param name="homeFolder">Home folder containing configuration and metadata.</param>
        private void Initialize(string configurationFolder)
        {
            DirectoryInfo dirInfo = new DirectoryInfo(configurationFolder);
            if (!dirInfo.Exists)
            {
                throw new ServiceProviderUtilityException(Resources.ServiceProviderUtilityHomeFolderNotFound + configurationFolder);
            }

            this.configurationFolder = configurationFolder;

            // Load the metadata for this service provider.
            this.ServiceProvider = new ServiceProvider(this.configurationFolder);
        ...
        {
            DirectoryInfo dirInfo = new DirectoryInfo(this.configurationFolder);
        ...
        private void InitializeIdentityProviders()
        {
            ...
            foreach (FileInfo metadataFile in files)
            {
                ...
                FileInfo extendedFile = new FileInfo(this.configurationFolder + @"/" + extendedFileName);
        ...
        public static void Login(HttpContext context, HttpRequest request, string configFolder)
        {
            string loginURL = string.Empty;
            ServiceProviderUtility serviceProviderUtility;
            serviceProviderUtility = new ServiceProviderUtility(context, configFolder);
        ...
        public static void LoginAndRedirect(HttpContext context, HttpResponse response, string configFolder, string portalURL, out string username, out Dictionary<string, string> logoutURLs)
        {
            username = string.Empty;
            AuthnResponse authnResponse = null;
            StringBuilder sloListItems;
            logoutURLs = new Dictionary<string, string>();
            ...
            else
            {
                ...
                {
                    if (key.ToUpper().Equals("AIRUUID"))
                    {
                        username = (string)((ArrayList)authnResponse.Attributes[key])[0];
                        //username = authnResponse.Attributes[key].ToString();
                    }
                }
            }

            GetLogoutURL(authnResponse, serviceProviderUtility, portalURL, out sloListItems, out logoutURLs);
        }

        private static void GetLogoutURL(AuthnResponse authnResponse, ServiceProviderUtility serviceProviderUtility, string portalURL, 
            out StringBuilder sloListItems, out Dictionary<string, string> logoutURLs)
        {
            ...
            sloListItems = new StringBuilder();
...
//                    idpUrl = HttpUtility.HtmlEncode(idpUrl);
                    ...
                    logoutURLs.Add("IDP_HttpRedirectProtocolBinding", idpUrl);
                    //sloListItems.Append(String.Format(sloListItemFormat, idpUrl, "Identity Provider", "HTTP Redirect"));
                    idpUrl = (String.Format(idpUrlFormat, idpDeployment, idpMetaAlias, Saml2Constants.HttpPostProtocolBinding, portalURL));
                    logoutURLs.Add("IDP_HttpPostProtocolBinding", idpUrl);
                    idpUrl = (String.Format(idpUrlFormat, idpDeployment, idpMetaAlias, Saml2Constants.HttpSoapProtocolBinding, portalURL));
                    logoutURLs.Add("IDP_HttpSoapProtocolBinding", idpUrl);
                }
            }
            ...
            spUrl = (String.Format(spUrlFormat, idp.EntityId, authnResponse.SubjectNameId, authnResponse.SessionIndex, Saml2Constants.HttpRedirectProtocolBinding, portalURL));
            logoutURLs.Add("SP_HttpRedirectProtocolBinding", spUrl);
            spUrl = (String.Format(spUrlFormat, idp.EntityId, authnResponse.SubjectNameId, authnResponse.SessionIndex, Saml2Constants.HttpPostProtocolBinding, portalURL));
            logoutURLs.Add("SP_HttpPostProtocolBinding", spUrl);
            spUrl = (String.Format(spUrlFormat, idp.EntityId, authnResponse.SubjectNameId, authnResponse.SessionIndex, Saml2Constants.HttpSoapProtocolBinding, portalURL));
            logoutURLs.Add("SP_HttpSoapProtocolBinding", spUrl);
        }


        public static void LogoutAndRedirect(HttpRequest request, HttpResponse response, HttpContext context,
            string configFolder, string portalURL, bool clearCookiesAndSession)
        {
            if (clearCookiesAndSession)
            {
                request.Cookies.Clear();
                context.Session.Clear();
            }
            ServiceProviderUtility serviceProviderUtility = new ServiceProviderUtility(context, configFolder);
            ...
            try
            {
                ...
                if (!String.IsNullOrEmpty(samlResponse))
                {
                    ...
                    else
                    {
                        ...
                        //Response.Write(fedletUrl);
                        response.Redirect(fedletUrl, false);
                    }
                }
                else if (!String.IsNullOrEmpty(samlRequest))
                {
                    ...
                    response.Write("obtain the logout request from IDP initiated SLO");
                    response.Write(context.Request.HttpMethod);
                    //response.End();
                    // send the logout response
                    string relayState = portalURL;
                    //serviceProviderUtility.SendLogoutResponse(Context, logoutRequest, relayState);
                    serviceProviderUtility.SendLogoutResponse(context, logoutRequest, relayState);
                }
                else
                {
