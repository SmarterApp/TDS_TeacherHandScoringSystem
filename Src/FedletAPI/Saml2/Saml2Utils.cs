...
namespace Sun.Identity.Saml2
{
    ...
    public static class Saml2Utils
    {
        ...
        public static string ConvertFromBase64Decompress(string message)
        {
            ...
            string decompressedMessage = string.Empty;
            using (StreamReader streamReader = new StreamReader(new DeflateStream(new MemoryStream(byteArray), CompressionMode.Decompress)))
            {

                // put in a string
                decompressedMessage = streamReader.ReadToEnd();
                streamReader.Close();
            }
        ...
        public static void EncryptAttributeQueryNameID(string certValue, string symmetricAlgorithmUri, XmlDocument xmlDoc)
        {
            ...
            if (string.IsNullOrEmpty(symmetricAlgorithmUri))
            {
